...
const MyAuctions = () => {
  ...
  useEffect(() => {
    ...
    const fetchMyAuctions = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/auctions/my", {
          method: "POST",
          headers: {
  ...
  const handleDeleteAuction = async (auctionId) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/auctions/delete", {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (data.success) {
        ...
        setMessage(data.message || "Failed to delete auction.");
        setMessage_ok(false);
      }
    } catch (err) {
  ...
  const statusChange = (auctionId, status) => {
    ...
    try {
      const response = fetch(
        "http://localhost:5000/api/auctions/statusUpdate",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
