...
const CreateAuction = () => {
  ...
  const uploadImage = async (image) => {
    ...
    const uploadResponse = await fetch(
      "http://localhost:5000/api/auctions/upload-image",
      {
        method: "POST",
        body: formDataImage,
      }
    ...
    return `http://localhost:5000/uploads/${uploadResult.data.fileName}`;
  };
      ...
      await Promise.all(
        additionalImages.map(async (imageData, index) => {
          if (images[index + 1]) {
            ...
            await fetch("http://localhost:5000/api/auctions/additem", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
